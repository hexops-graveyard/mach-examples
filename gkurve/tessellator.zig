...
pub const Tessellator = struct {
    ...
    pub fn triangulatePolygons2(self: *Tessellator, pts: []const Vec2, polygons: []const IndexSlice(u32)) void {
        for (polygons) |slice| {
...
fn edgeToString(edge: Edge) []const u8 {
    ...
    return std.fmt.bufPrint(&S.buf, "{} ({},{}) -> {} ({},{})", .{ edge.start_idx, edge.start_pos[0], edge.start_pos[1], edge.end_idx, edge.end_pos[0], edge.end_pos[1] }) catch unreachable;
}
...
pub const Edge = struct {
    ...
    fn init(start_idx: u16, start_v: InternalVertex, end_idx: u16, end_v: InternalVertex) Edge {
        var new = Edge{
            ...
            .end_pos = end_v.pos,
            .vec = Vec2{ end_v.pos[0] - start_v.pos[0], end_v.pos[1] - start_v.pos[1] },
            .x_slope = undefined,
            .is_horiz = undefined,
        };
        if (new.vec[1] != 0) {
...
fn testCountMulti(polygons: []const []const f32, exp_tri_count: u32) !void {
    ...
    var polygons_buf = std.ArrayList(IndexSlice(u32)).init(t.alloc);
